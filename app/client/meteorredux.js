...
function MeteorRedux(store) {
  ...
  this.addBinding = function addBinding(reducer, assigner) {
    let bindingId = getUniqueID();
    this._bindings.push({ reducer, assigner, calculateData, bindingId, store: this });
    return bindingId;
  };

  this.dispatchAction = function dispatchAction(bindingId) {
    if (this._store) {
      this._store.dispatch({ type: MeteorReduxActionName, bindingId });
    }
  };

  this.findBindingByReducer = function findBindingByReducer(reducer) {
    let result = null;
